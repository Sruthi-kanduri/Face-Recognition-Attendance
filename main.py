...
def TakeImages():
    ...
    # Check if the student details file exists and get the current serial number
    serial = 0
    ...
    # Get the ID and Name from the input fields
    Id = (txt.get())
    name = (txt2.get())

    # Check if the name is valid (only alphabets and spaces)
    if ((name.isalpha()) or (' ' in name)):
        ...
        if exists:
            # Display message if the student ID already exists
            res = f"Student with ID {Id} is already registered."
            message1.configure(text=res)
        else:
            # Proceed with registering the student and taking images
